...
public class StateMachinePlantUMLTest {
    private static final Log LOGGER = LogFactory.getLog(StateMachinePlantUMLTest.class);
    ...
    public void testPlantUML(){
        ...
        Stream.of(P0_Changed, Page_Price_changed)
                .forEach(event -> builder.externalTransition()
                        .from(Supplier_Processing, Supplier_Manager_Processing,
                                Price_Manager_Processing)
                        .to(Closed)
                        .on(event)
                        .when(checkCondition())
        ...
        StateMachine<PriceAdjustmentTaskStatusEnum, PriceAdjustmentTaskEventEnum> stateMachine =builder.build("AdjustPriceTask");
        String plantUML = stateMachine.generatePlantUml();
        LOGGER.info(plantUML);
    }

    private Condition<PriceAdjustmentTaskStatusEnum, PriceAdjustmentTaskEventEnum> checkCondition() {
        return (ctx) -> {return true;};
    }

    private Action<PriceAdjustmentTaskStatusEnum, PriceAdjustmentTaskEventEnum> doAction() {
        return (ctx)->{
        };
    }
}
